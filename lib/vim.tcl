...
namespace eval vim {
  ...
  proc multicursor_mode {txtt} {
    ...
    # Indicate that multimove is enabled
  ...
  proc disable_multicursor {txtt} {
    ...
    $txtt configure -blockcursor [expr {$mode($txtt) ne "edit"}] -multimove 0
  ...
  proc handle_find_motion {txtt char} {
    ...
    set cursorargs "none"
    if {$dir eq "prev"} {
      set cursorargs [list findchar -dir prev -char $char -num [get_number $txtt] -exclusive $excl]
    }

    if {($operator($txtt) eq "") || ($dir eq "prev")} {
      return [do_operation $txtt [list findchar -dir $dir -char $char -num [get_number $txtt] -exclusive $excl] cursor -cursor $cursorargs]
    } else {
  ...
  proc handle_asterisk {txtt} {
    ...
    if {$mode($txtt) eq "command"} {
      set txt [winfo parent $txtt]
      if {[search::find_current_word $txt]} {
        search::find_next $txt
      }
    }
