...
namespace eval select {
    ...
    switch $motion {
      ...
      prev {
        ...
        switch $data($txtt,type) {
          ...
          default {
            ...
            if {$opts(-startpos) ne ""} {
              lset range $index [$txtt index {*}[lindex $pos $index] -dir $motion -num $number -startpos $opts(-startpos)]
            } else {
              lset range $index [$txtt index {*}[lindex $pos $index] -dir $motion -num $number -startpos [lindex $range $index]]
            }
          }
        }
      ...
      lshift {
        if {$data($txtt,type) eq "block"} {
          ...
          foreach index {0 1} {
            lset range $index [$txtt index {*}[lindex $pos $index] -dir $dir -num $number -startpos [lindex $range $index]]
          }
        }
      }
      ushift {
  ...
  proc node_parent {txt startpos endpos} {
    ...
    if {(([set tag [ctext::inside_tag $txt -startpos $startpos -allow010 1]] eq "") && ([lindex $tag 3] ne "010")) || \
      ...
      return [ctext::get_outer $within]
    } else {
      return [ctext::get_inner $within]
    }
  ...
  proc node_next_sibling {txt startpos} {
    ...
    if {[set tag [ctext::get_tag $txt -dir next -type ??0 -start [lindex $current_range 1]]] ne ""} {
  ...
  proc node_prev_sibling {txt startpos} {
    ...
    if {[set tag [ctext::get_tag $txt -dir prev -type 0?? -start "[lindex $current_range 0]-1c"]] ne ""} {
  ...
  proc bracket_last_child {txtt type startpos endpos} {
    ...
    if {[$txtt is $type right $endpos-1c]} {
