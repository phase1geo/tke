namespace eval snippets {
  ...
  proc run_test19 {} {
    ...
    clipboard clear
    clipboard append "copied text"

    snippets::insert_snippet $txt.t {Check out my ${CLIPBOARD:aw snap}}

    if {[$txt get 1.0 end-1c] ne {Check out my copied text}} {
      cleanup "Insertion from clipboard did not work ([$txt get 1.0 end-1c])"
    }

    # Add some strings to the clipboard history
    foreach str [list text3 text2 text1] {
      clipboard clear
      clipboard append $str
      cliphist::add_from_clipboard
    }

    # Make sure that everything is coming from history
    clipboard clear

    # Verify clipboard history entry 2
    $txt delete 1.0 end
    snippets::insert_snippet $txt.t {Cliphist 3 is ${CLIPHIST[2]:no good}}

    if {[$txt get 1.0 end-1c] ne {Cliphist 3 is text3}} {
      cleanup "Insertion from clipboard history 3 is wrong ([$txt get 1.0 end-1c])"
    }

    # Verify clipboard history entry 1
    $txt delete 1.0 end
    snippets::insert_snippet $txt.t {Cliphist 2 is ${CLIPHIST[1]:no good}}

    if {[$txt get 1.0 end-1c] ne {Cliphist 2 is text2}} {
      cleanup "Insertion from clipboard history 2 is wrong ([$txt get 1.0 end-1c])"
    }

    # Verify last from clipboard history
    $txt delete 1.0 end
    snippets::insert_snippet $txt.t {Cliphist 1 is ${CLIPHIST[0]:no good}}

    if {[$txt get 1.0 end-1c] ne {Cliphist 1 is text1}} {
      cleanup "Insertion from clipboard history 3 is wrong ([$txt get 1.0 end-1c])"
    }
